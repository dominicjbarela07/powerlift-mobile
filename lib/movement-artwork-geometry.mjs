...
export function movementHeroGeometry(width, height, focal) {
  if (focal.cropMode === 'contain') {
    // Keep full machines/inverted poses intact, at the same visual weight as
    // focal art. Containment controls geometry, never a separate photo-tile
    // treatment: the shared hero scrim integrates both into the canvas.
    const size = Math.max(0, Math.min(360, width * .60, height * .76));
