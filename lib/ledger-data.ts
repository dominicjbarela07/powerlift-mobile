...
import { formatCalculatedWeightValue, kilogramsToDisplayValue, type DisplayWeightUnit } from '@/lib/display-units';
...
export function displayCalculatedWeight(
  valueKg: number | null | undefined,
  unit: LedgerUnit,
  fallback = '—',
): string {
  if (valueKg === null || valueKg === undefined || !Number.isFinite(valueKg)) return fallback;
  return formatCalculatedWeightValue(
    kilogramsToDisplayValue(valueKg, unit as DisplayWeightUnit),
    unit as DisplayWeightUnit,
  ) ?? fallback;
}
