 ...
 */
export async function fetchLedgerAccomplishmentHistory(maxPages = 20, athleteId?: number, eventTypes?: readonly string[]): Promise<AccomplishmentEvent[]> {
  const items: AccomplishmentEvent[] = [];
  let cursor: string | null = null;

  for (let pageIndex = 0; pageIndex < Math.max(1, maxPages); pageIndex += 1) {
    const page = await fetchLedgerAccomplishmentPage(50, cursor, athleteId, undefined, eventTypes);
