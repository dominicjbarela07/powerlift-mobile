...
export function sessionTimePartsInZone(value: Date, timeZone: string): ZonedSessionTimeParts | null {
  return timeZoneParts(value, timeZone);
}

/** Convert wall-clock parts in an IANA timezone into one unambiguous instant. */
export function sessionTimePartsToInstant(parts: ZonedSessionTimeParts, timeZone: string): Date | null {
...
export function replaceSessionDatePart(current: Date, selectedDate: Date, timeZone: string): Date | null {
