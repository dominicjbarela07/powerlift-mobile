...
- `KeyboardViewport` measures its native view in window coordinates and removes only the overlapping keyboard area. The root native Stack applies it per scene. `KeyboardModal` creates an independent owner for each native modal because React context crosses native modal portals. Each native presentation resets inherited form-focus ownership, so a modal input cannot scroll the underlying screen. Already resized Android windows have zero additional overlap.
