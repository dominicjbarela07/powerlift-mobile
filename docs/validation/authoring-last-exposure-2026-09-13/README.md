...
Native DEV: the original false state and corrected `125 lb × 7 @8.5 RPE`, `Sep 9`, `Representative set · 4 recorded sets` were observed in actual Simulator accessibility state from canonical Metro. The stored viewport screenshot shows the editor above this section; it is not evidence that the below-fold history value is visible in that image. The Core History action was invoked, but the Mac locked before its destination/return could be observed. Additional physical self/workspace, scrolling/background and History-return checks are pending; component, cache, route and authenticated endpoint contracts pass. No physical interaction pass is claimed for those pending cases. This is a narrow data-resolution repair, not a visual redesign.
