...
print(json.dumps({'kept':1,'accepted_new':len(accepted),'pending':50-len(accepted)}))
...
baseline = next(r for r in manifest if r['id']==33)
...
for group_index in range(0,len(ordered),6):
 group = [baseline]+ordered[group_index:group_index+6]
