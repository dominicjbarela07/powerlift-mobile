...
root=Path.cwd()
proof=root/"powerlift_mobile/docs/validation/free-weight-push-family-2026-09-11"
...
with app.app_context():
 ...
 if mode=="create":
  ...
  by_id={r["id"]:r for r in audit["qualifying"]}
  ids=[33,79,80,84,103,289,292,297]
  payload={"athlete_id":12,"date":"2026-09-11","label":"Free-weight Push Art QA","status":"draft","core_items":[],"acc_items":[{"movement_definition_id":i,"movement":by_id[i]["display_name"],"sets":3,"reps_text":"10-12","rir_target":2} for i in ids]}
  ...
  for primary in ("chest","triceps","front_delts","side_delts"):
   ...
   expected=sorted(r["id"] for r in audit["qualifying"] if r["primary_muscle_group"]==primary)
