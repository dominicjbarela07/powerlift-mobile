...
root=Path.cwd()
proof=root/"powerlift_mobile/docs/validation/free-weight-pull-family-2026-09-11"
...
with app.app_context():
 ...
 if mode=="create":
  ...
  push=json.loads((root/"powerlift_mobile/docs/validation/free-weight-push-family-2026-09-11/taxonomy-audit.json").read_text())
  by_id={r["id"]:r for r in audit["qualifying"]+push["qualifying"]}
  ids=[33,191,80,154,194,126,235,155,545]
  payload={"athlete_id":12,"date":"2026-09-11","label":"Free-weight Push + Pull Art QA","status":"draft","core_items":[],"acc_items":[{"movement_definition_id":i,"movement":by_id[i]["display_name"],"sets":3,"reps_text":"10-12","rir_target":2} for i in ids]}
  ...
  for primary in ("lats","upper_back","rear_delts","traps","lower_back"):
   ...
   expected=sorted(r["id"] for r in audit["candidates"] if r["primary_muscle_group"]==primary)
   assert sorted(r["id"] for r in items)==expected,(primary,[r["id"] for r in items])
   assert not data.get("next_cursor")
   results.append({"primary":primary,"http_status":response.status_code,"request":params,"items":[{k:v for k,v in row.items() if k in fields} for row in items]})
