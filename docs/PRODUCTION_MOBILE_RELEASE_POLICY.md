...
- This historical `main` checkout is retired as a publication source. Its
  `scripts/eas-update-production.sh` entry point fails closed and directs release
  work to a clean governed Production 2.0.2 projection from canonical source.
- `scripts/eas-update-production.sh` delegates only to the guarded dual-platform
  publisher in a governed release projection.
