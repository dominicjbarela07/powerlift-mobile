...
assert.equal(recognitionPresentation(volumeMilestone(541, 'CORE_LIFETIME_VOLUME_MILESTONE'), 'kg'), null, 'An LB award cannot become a KG award');
const kgEvent = volumeMilestone(542, 'CORE_LIFETIME_VOLUME_MILESTONE');
kgEvent.evidence = {...kgEvent.evidence, milestone_unit: 'kg', threshold_value: 100_000, threshold_lb: null, threshold_kg: 100_000, new_total_kg: 100_080, next_threshold_value: 250_000};
const liftLandmarkKg = recognitionPresentation(kgEvent, 'kg');
assert.equal(liftLandmarkKg.eyebrow, 'SQUAT LIFETIME VOLUME LANDMARK');
assert.equal(liftLandmarkKg.value, '100K KG');
assert.match(liftLandmarkKg.detail, /100,080 KG accumulated/);
