...
assert.match(route, /data.workout.status === 'in_progress'\) return;[\s\S]*stopRestTimer\(\)/);
...
assert.match(route, /restTimer=\{activeRestTimer\}/);
...
assert.match(sheet, /FEEDBACK_OPTIONS = \[[\s\S]*key: 'flies'[\s\S]*key: 'expected'[\s\S]*key: 'heavy'[\s\S]*key: 'very_heavy'/);
assert.match(sheet, /testID=\{`smart-warmup-feedback-\$\{feedback\.key\}`\}/);
assert.doesNotMatch(sheet, /\(\['slow', 'expected', 'fast'\] as const\)/);
assert.match(sheet, /Haptics\.notificationAsync\(Haptics\.NotificationFeedbackType\.Success\)/);
assert.match(sheet, /future warmup\$\{remaining === 1 \? '' : 's'\} updated/);
assert.match(sheet, /Warmup ramp confirmed/);
assert.match(sheet, /accessibilityLiveRegion="polite" style=\{styles\.adaptationNotice\}/);
...
assert.match(sheet, /PROTECT THE WORKING SET TODAY/);
assert.match(sheet, /recommendation\?\.explanation/);
