...
requireMatch(workspace, /bottom=\{insets\.bottom \+ SLSpacing\.xs \+ SL_TAB_ROW_CONTROL\.shellHeight \+ SLSpacing\.md\}[\s\S]*sessionToolkit: \{ position: 'absolute', right: SLLayout\.screenGutter/, 'the floating toolkit must sit one full control height and one spacing token above the bottom tab row.');
...
requireMatch(workspace, /const ACCESSORY_CATEGORY_ARTWORK = require\(\s*'@\/assets\/images\/movement-category-artwork-v5\/accessory-wordmark-coin-seal\.png',?\s*\)/, 'the workspace must reuse the canonical Session Logger accessory artwork.');
requireMatch(workspace, /if \(kind === 'accessory'\)[\s\S]*source=\{ACCESSORY_CATEGORY_ARTWORK\}[\s\S]*const identity = resolveLoggerLiftIdentity\(item\)/, 'accessory classification must select canonical Logger artwork before core lift-name inference.');
...
requireMatch(tabLayout, /includes\('\/workout\/session-workspace\/'\)[\s\S]*sceneStyle: usesRouteOwnedHorizontalLayout \? styles\.fullBleedTabScene : styles\.tabScene/, 'the Session Workspace must not inherit an additional shell gutter around its route-owned layout.');
