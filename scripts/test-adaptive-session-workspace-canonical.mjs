...
requireMatch(workspace, /bottom=\{props\.sheetPresentation[\s\S]*?SLSpacing\.md[\s\S]*?: insets\.bottom \+ SLSpacing\.md[\s\S]*sessionToolkit: \{ position: 'absolute', right: SLLayout\.screenGutter/, 'the floating toolkit must use the one deterministic lower-right slot after workspace navigation is suppressed.');
requireMatch(workspace, /setSessionEditorOverlayOpen\(true\)[\s\S]*setSessionEditorOverlayOpen\(false\)/, 'the standalone workspace must suppress the competing tab shell for its complete mounted lifecycle.');
