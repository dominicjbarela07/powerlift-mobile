...
const skippedSecond = skipSequentialGroupStep(advanced.state);
assert.equal(skippedSecond.entries[1].skipped, true);
assert.equal(
  validateSequentialGroupForSave(skippedSecond).validation.valid,
  true,
  'an explicitly skipped superset movement must preserve round integrity without requiring fake log data',
);
const resumedSecond = updateSequentialGroupDraft(skippedSecond, 2, 'reps', '14');
assert.equal(resumedSecond.entries[1].skipped, false, 'editing a skipped movement resumes individual logging');
...
assert.match(modalSource, />Skip</);
assert.match(modalSource, /Log Individually/);
...
console.log(
  '[sequential-group-logger] navigation, skips, individual logging, draft retention, validation routing, partial recovery, single-form rendering, save guards, and explicit contrast passed',
);
