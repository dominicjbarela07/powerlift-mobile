...
assert.match(
  workoutLoggerSource,
  /manufacturerName=\{\s*equipmentPickerManufacturer\.manufacturer\?\.display_name \|\| 'Other'\s*\}/,
  'The selected Production 2.0.2 manufacturer must render the canonical brand mark',
);
assert.match(
  workoutLoggerSource,
  /manufacturerName=\{row\.manufacturer\?\.display_name \|\| 'Other'\}/,
  'Every Production 2.0.2 picker row must render its canonical brand mark',
);
assert.match(
  workoutLoggerSource,
