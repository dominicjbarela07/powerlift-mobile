...
assert.equal(
  resolveCanonicalMovementArtwork({ kind: 'accessory', movement_identity: { id: 9001, primary_muscle_group: 'biceps' } }).regionKey,
  'biceps',
  'Canonical primary muscle must win over family and display-name inference.',
);
assert.equal(
  resolveCanonicalMovementArtwork({ kind: 'custom', movement_identity: { id: 9002, primary_muscle_group: 'biceps' } }).regionKey,
  'biceps',
  'Custom movement artwork must use its saved primary muscle.',
);
assert.deepEqual(resolveCanonicalMovementArtwork({ kind: 'accessory' }), { kind: 'neutral', reason: 'missing_canonical_identity' }, 'Identity-free legacy prescriptions must fail closed to neutral artwork.');
...
assert.match(workspace, /function MovementArtwork[\s\S]*<CanonicalMovementArtwork/, 'Session Workspace must use the one governed individual-movement artwork resolver.');
