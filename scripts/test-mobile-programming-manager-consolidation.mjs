...
assert.match(manager, /<FocusedSessionAuthoring[\s\S]*?<MobileSessionWorkspaceContent/, 'Session tap must open the canonical embedded workspace');
assert.match(manager, /accessibilityLabel=\{`Week \$\{selectedWeek\.index\} actions`\}/, 'Week actions must remain on the main workspace');
assert.match(manager, /<SwipeActionRow[\s\S]*?onAction=\{move\}/, 'Session rows must use the canonical swipe primitive');
