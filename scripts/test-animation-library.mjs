...
assert.match(preview, /entry\.id === 'weight-pr'[\s\S]*WeightPrRecognitionPreview/, 'Weight PR must use its isolated DEV choreography');
assert.match(preview, /accessibilityLabel="Former best weight"[\s\S]*accessibilityLabel="New best weight"/, 'Weight PR must expose movement and load parameters');
assert.match(preview, /accessibilityLabel="Rep-Max rep count"/, 'Rep-Max must expose its rep-count parameter');
...
assert.match(majorMilestone, /MajorVolumeMilestoneArtifact[\s\S]*<MedallionImage[\s\S]*majorVolumeMedallionAsset/s, 'the landmark must resolve through the canonical static medallion library');
...
assert.equal((majorMilestoneAssets.match(/require\('@\/assets\/images\/major-volume-medallions\//g) || []).length, 37, 'the LB registry must preserve its 37 original static assets; KG tiles are validated separately');
...
assert.match(preview, /oldTranslateY[\s\S]*toValue: -displacementDistance[\s\S]*newTranslateY[\s\S]*toValue: 0/s, 'the new record must physically displace the former best');
assert.match(preview, /1 · Former best established[\s\S]*2 · Challenger approaches[\s\S]*3 · Displacement impact[\s\S]*4 · Victory moment[\s\S]*5 · Settle and breathe[\s\S]*6 · Evidence reveal begins[\s\S]*7 · Complete comparison[\s\S]*8 · Final settled state/s, 'Weight PR must implement all eight storyboard phases in order');
assert.match(preview, /FORMER BEST[\s\S]*NEW BEST[\s\S]*RECORD REPLACED/s, 'Weight PR must tell the replacement story in its visible hierarchy');
assert.match(preview, /weightPrComparison[\s\S]*arrow-forward[\s\S]*weightPrDelta/s, 'the settled comparison and improvement must render as one evidence unit');
...
assert.match(preview, /if \(settings\.reduceMotion\)[\s\S]*fragmentOpacity\.setValue\(0\)[\s\S]*evidenceOpacity\.setValue\(1\)[\s\S]*8 · Final settled state/s, 'Reduced Motion must remove atmosphere and show the final factual comparison immediately');
