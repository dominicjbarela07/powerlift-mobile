...
assert.match(previewSource, /FORMER \$\{repCountValue\} REP MAX/);
assert.match(previewSource, /NEW \$\{repCountValue\} REP MAX/);
assert.match(previewSource, /evidenceLabel = isRepMax \? `\$\{repCountValue\} REP MAX`/);
assert.match(previewSource, /adjustsFontSizeToFit numberOfLines=\{1\} style=\{\[styles\.weightPrNewLabel/);
assert.match(previewSource, /adjustsFontSizeToFit numberOfLines=\{1\} style=\{\[styles\.weightPrEvidenceLabel/);
assert.match(previewSource, /accessibilityLabel="Former best weight"/);
assert.match(previewSource, /accessibilityLabel="New best weight"/);
assert.match(previewSource, /accessibilityLabel="Rep-Max rep count"/);
assert.match(previewSource, /\[.*playKey.*\]/s);
assert.doesNotMatch(previewSource, /fixed=\{\{ movement:/);
assert.match(previewSource, /<SLTrophy size=\{24\}/);
assert.match(previewSource, /<SLTrophy size=\{54\}/);
...
assert.match(sharedPresentationSource, /repMaxEvidenceTitle/);
