...
for (const platform of platforms) {
  ...
  const requiredRouteMarkers = ['RootLayout', 'TabsLayout', 'WorkoutViewerScreen'];
  const missingMarkers = requiredRouteMarkers.filter(
    (marker) => !bundle.includes(Buffer.from(marker)),
  );
  if (bundle.length < 5_000_000 || missingMarkers.length) {
    throw new Error(
      `OTA blocked: exported ${platform} 2.0.2 bundle does not contain the application route tree `
      + `(bytes=${bundle.length}, missing=${missingMarkers.join(', ') || 'none'}).`,
    );
  }
  console.log(
    `OTA route bundle PASS — ${platform}/${path.basename(bundles[0])}, ${bundle.length} bytes, `
    + '2.0.2 canonical routes present.',
  );
}
