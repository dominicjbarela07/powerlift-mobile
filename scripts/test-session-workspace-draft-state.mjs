...
assert.match(workspace, /const saveWorkspaceChanges[\s\S]*onSaveSession\(buildSessionWorkspaceSavePlan\(sessionDraft, persistedSession\)\)[\s\S]*if \(!success\)[\s\S]*return false[\s\S]*setPersistedSession/, 'failed saves retain the draft and successful saves establish a clean snapshot');
