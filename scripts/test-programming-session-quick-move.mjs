...
assert.match(source, /<SwipeActionRow[\s\S]*?onAction=\{runSwipeAction\}/, 'the live in-place Week workspace must use the canonical swipe primitive');
assert.match(source, /const runSwipeAction = \(\) => \{[\s\S]*?if \(movable\)[\s\S]*?onQuickMoveSession\(session\)/, 'editable Sessions must expose quick Move without making completed evidence movable');
