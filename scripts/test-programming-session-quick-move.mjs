...
assert.match(source, /accessibilityLabel="Training Program actions"/);
assert.match(source, /accessibilityLabel=\{`Week \$\{selectedWeek\.index\} actions`\}/);
assert.match(source, /<SwipeActionRow[\s\S]*?onAction=\{move\}/, 'the live in-place Week workspace must use the canonical swipe primitive');
assert.match(source, /const move = \(\) => \{[\s\S]*?movable \? onQuickMoveSession\(session\) : onSessionActions\(session, context\)/, 'editable Sessions must expose quick Move without making completed evidence movable');
assert.doesNotMatch(source, /useState<'overview' \| 'week'>/, 'Session move must not depend on a redundant Week screen');
