...
export function CanonicalMovementHistoryScreen({
  ...
  const assistanceHistory = history?.strength_metric.key === 'assistance';
    ...
    <>
      ...
      >
        {loading && !history ? <State icon="hourglass-outline" title="Loading exact movement evidence" /> : error && !history ? <State icon="alert-circle-outline" title={error} action="Try again" onAction={() => void load(false)} /> : history ? (
          <>
            ...
            <NumberedSection number="1" title={assistanceHistory ? 'ASSISTANCE TREND' : 'PERFORMANCE TREND'} subtitle={assistanceHistory ? `Less assistance means stronger performance · best ${unknownEquipmentSeries ? 'recorded' : 'comparable'} set` : `${history.strength_metric.short_label} from best ${unknownEquipmentSeries ? 'recorded' : 'comparable'} set`}>
              ...
              <AnalyticalHistoryChart points={history.performance_trend} metric="strength" metricLabel={history.strength_metric.short_label} unit={unit} color="#A865FF" estimated={!assistanceHistory} xDomainMode={historyAxisMode} onOpenExposure={(id) => void openExposure(id)} />
              <Text style={styles.chartFootnote}>{unknownEquipmentSeries ? 'Unknown-equipment historical series · not compared with named implementations · ' : ''}{assistanceHistory ? 'Recorded assistance · lower is better · reps and effort remain visible per exposure' : 'Canonical Epley estimate · recorded RPE, or recorded RIR when RPE is absent'}</Text>
            </NumberedSection>

            {!assistanceHistory ? <NumberedSection number="2" title="LOAD PROGRESSION" subtitle={unknownEquipmentSeries ? 'Best recorded set over time' : 'Best comparable set over time'} tone="#EF3C8C">
            ...
            <NumberedSection number={assistanceHistory ? '2' : '3'} title={assistanceHistory ? 'ASSISTANCE × REP PROFILE' : 'LOAD × REP PROFILE'} subtitle="Demonstrated performance envelope" tone="#9B6BDB">
              <LoadRepProfile points={history.load_rep_profile} unit={unit} seriesLabel={profileSeriesLabel} loadLabel={assistanceHistory ? 'Assistance' : 'Load'} onOpenExposure={(id) => void openExposure(id)} />
            </NumberedSection>

            <Section title="KEY STATISTICS">
              <StatisticsGrid statistics={history.statistics} metricLabel={history.strength_metric.short_label} unit={unit} assistancePoints={assistanceHistory ? history.performance_trend : null} onOpenExposure={(id) => void openExposure(id)} />
            </Section>
      ...
      <StrengthLedgerBottomSheet accessibilityLabel="Exposure Details" heightFraction={0.91} visible={Boolean(selectedExposureId)} onDismiss={closeExposure} onRequestClose={closeExposure}>
        <ExposureDetailContent
          ...
          error={detailError}
          unit={unit}
          metricLabel={history?.strength_metric.short_label || 'e10RM'}
          estimated={history?.strength_metric.key !== 'assistance'}
...
function StatisticsGrid({ statistics, metricLabel, unit, assistancePoints, onOpenExposure }: { statistics: CanonicalMovementHistory['statistics']; metricLabel: string; unit: MovementHistoryUnit; assistancePoints: CanonicalMovementHistory['performance_trend'] | null; onOpenExposure: (id: string) => void }) {
  ...
  if (assistancePoints) {
    const lowest = assistancePoints
      .filter((point) => point.weight_kg != null && Number.isFinite(Number(point.weight_kg)))
      .reduce<CanonicalMovementHistory['performance_trend'][number] | null>((best, point) => !best || Number(point.weight_kg) < Number(best.weight_kg) ? point : best, null);
    return <View style={styles.statisticsGrid}><StatisticCard tone="#38D381" icon="trending-down-outline" label="LOWEST ASSISTANCE" value={lowest ? `${displayWeight(lowest.weight_kg, unit)} ${unit}` : '—'} detail={lowest ? dateLabel(lowest.date) : 'No exact assistance evidence'} subdetail={lowest ? 'Lower assistance is stronger' : null} onPress={lowest ? () => onOpenExposure(lowest.exposure_id) : undefined} /><StatisticCard tone="#A865FF" icon="analytics-outline" label="PROGRESSION METRIC" value="Assistance required" detail="Recorded load · not estimated RM" /><StatisticCard tone="#2CC8B7" icon="repeat-outline" label={`REP PR${rep ? ` @ ${displayWeight(rep.weight_kg, unit)} ${unit} assistance` : ''}`} value={rep ? `${rep.reps} reps` : '—'} detail={rep?.previous_reps != null ? `Previous: ${rep.previous_reps} reps` : 'No prior same-assistance match'} /><StatisticCard tone="#E4A92F" icon="shield-checkmark-outline" label="COMPARISON POLICY" value="Less is stronger" detail="Reps and effort must also agree" /></View>;
  }
  return <View style={styles.statisticsGrid}><StatisticCard tone="#A865FF" icon="trending-up-outline" label={`EST. STRENGTH (${metricLabel})`} value={strength ? formatCalculatedWeightFromKg(strength.value_kg, unit) || '—' : '—'} detail={strength ? dateLabel(strength.date) : 'No exact estimate'} subdetail={strength?.delta_kg ? `${formatCalculatedWeightDeltaFromKg(strength.delta_kg, unit, 'signed')} vs first exposure` : null} onPress={strength ? () => onOpenExposure(strength.exposure_id) : undefined} /><StatisticCard tone="#ED3F8E" icon="ribbon-outline" label="LOAD PR" value={load ? setLabel(load, unit) : '—'} detail={load ? dateLabel(load.date) : 'No load evidence'} /><StatisticCard tone="#2CC8B7" icon="repeat-outline" label={`REP PR${rep ? ` @ ${displayWeight(rep.weight_kg, unit)} ${unit}` : ''}`} value={rep ? `${rep.reps} reps` : '—'} detail={rep?.previous_reps != null ? `Previous: ${rep.previous_reps} reps` : 'No prior rep match'} /><StatisticCard tone="#E4A92F" icon="barbell-outline" label={bestN ? `BEST ${bestN.target_reps}-REP LOAD` : 'REP-RANGE LOAD'} value={bestN ? `${displayWeight(bestN.weight_kg, unit)} ${unit} × ${bestN.reps}` : '—'} detail={bestN ? 'Exact performed evidence' : 'No matching exposure'} /></View>;
}
...
function ExposureDetailContent({ detail, loading, error, unit, metricLabel, estimated, onViewSession }: { detail: CanonicalHistoryExposureDetail | null; loading: boolean; error: string | null; unit: MovementHistoryUnit; metricLabel: string; estimated: boolean; onViewSession: (id: number) => void }) {
