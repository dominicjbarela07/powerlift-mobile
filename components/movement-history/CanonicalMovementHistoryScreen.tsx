        ...
        {loading && !history ? <State icon="hourglass-outline" title="Loading exact movement evidence" /> : error && !history ? <State icon="alert-circle-outline" title={error} action="Try again" onAction={() => void load(false)} /> : history ? (
          <>
            ...
            <Section title="KEY STATISTICS">
              <StatisticsGrid statistics={history.statistics} metricLabel={history.strength_metric.short_label} unit={unit} onOpenExposure={(id) => void openExposure(id)} />
            </Section>
...
function StatisticsGrid({ statistics, metricLabel, unit, onOpenExposure }: { statistics: CanonicalMovementHistory['statistics']; metricLabel: string; unit: MovementHistoryUnit; onOpenExposure: (id: string) => void }) {
...
function ExposureDetailContent({ detail, loading, error, unit, metricLabel, onViewSession }: { detail: CanonicalHistoryExposureDetail | null; loading: boolean; error: string | null; unit: MovementHistoryUnit; metricLabel: string; onViewSession: (id: number) => void }) {
