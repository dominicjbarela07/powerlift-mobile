...
function PlanComparisonCard({ row, index, expanded, unit, onToggle, onOpenHistory }: { row: PlanComparisonRow; index: number; expanded: boolean; unit: DisplayWeightUnit; onToggle: () => void; onOpenHistory?: (movement: CompletedRecapMovement) => void }) {
  ...
  return <View style={[styles.compareMovementCard, expanded && styles.compareMovementCardExpanded]}>
    <Pressable accessibilityRole="button" accessibilityState={{ expanded }} accessibilityLabel={`${expanded ? 'Collapse' : 'Expand'} ${title} comparison`} onPress={onToggle} style={({ pressed }) => [styles.compareMovementHeader, pressed && styles.pressed]}>
      <View style={styles.compareIndex}><Text style={styles.compareIndexText}>{String(index + 1).padStart(2, '0')}</Text></View>
      ...
      {equipment[0]?.manufacturer ? <View style={styles.compareManufacturer}><ManufacturerBrandMark manufacturerName={equipment[0].manufacturer} /></View> : null}
...
function PlanCompareExperience({ recap, performedMovements, unit, onOpenHistory }: { recap: CompletedSessionRecapPayload; performedMovements: CompletedRecapMovement[]; unit: DisplayWeightUnit; onOpenHistory?: (movement: CompletedRecapMovement) => void }) {
  ...
  return <>
    ...
    <View style={styles.compareMovementStack}>{filtered.map((row) => <PlanComparisonCard key={row.key} row={row} index={rows.indexOf(row)} expanded={expandedKey === row.key} unit={unit} onToggle={() => setExpandedKey((current) => current === row.key ? null : row.key)} onOpenHistory={onOpenHistory} />)}{!filtered.length ? <View style={styles.emptyCard}><Ionicons name="filter-outline" size={25} color={SLColors.textMuted} /><Text style={styles.emptyTitle}>No movements in this filter</Text><Text style={styles.emptyBody}>The selected comparison state does not occur in this Session.</Text></View> : null}</View>
...
const styles = StyleSheet.create({
  ...
  compareMovementCard: { overflow: 'hidden', borderRadius: SLRadius.xl, borderWidth: 1, borderColor: '#30343E', backgroundColor: '#06080C', ...SLShadows.level1 }, compareMovementCardExpanded: { borderColor: '#4B2A65' }, compareMovementHeader: { minHeight: 91, flexDirection: 'row', alignItems: 'center', padding: 9 }, compareIndex: { alignSelf: 'flex-start', minWidth: 26, height: 26, alignItems: 'center', justifyContent: 'center', borderRadius: 13, borderWidth: 1, borderColor: '#432952', backgroundColor: '#0C0910' }, compareIndexText: { color: '#C878FF', fontFamily: SLFontFamilies.display, fontSize: 8 }, compareArtwork: { width: 58, height: 66, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', marginLeft: 5, borderRadius: 10, borderWidth: 1, borderColor: '#372643', backgroundColor: '#100A16' }, compareArtworkImage: { width: 56, height: 64 }, compareManufacturer: { width: 46, height: 48, alignItems: 'center', justifyContent: 'center', marginLeft: 5, borderRadius: 9, borderWidth: 1, borderColor: '#2C3039', backgroundColor: '#080A0F' }, compareMovementCopy: { flex: 1, minWidth: 0, marginLeft: 7 }, compareMovementTitle: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 12, lineHeight: 16 }, compareEquipment: { marginTop: 3, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 8 }, comparePrescription: { alignSelf: 'flex-start', marginTop: 5, overflow: 'hidden', paddingHorizontal: 5, paddingVertical: 3, borderRadius: 5, color: '#CE7FFF', backgroundColor: '#21102D', fontFamily: SLFontFamilies.bodyBold, fontSize: 7.2 }, compareStateWrap: { width: 73, alignSelf: 'stretch', alignItems: 'flex-end', justifyContent: 'space-between', marginLeft: 4 }, compareStateBadge: { maxWidth: 73, paddingHorizontal: 5, paddingVertical: 5, borderRadius: 11, borderWidth: 1 }, compareStateText: { fontFamily: SLFontFamilies.bodyBold, fontSize: 6.2, letterSpacing: 0.25, textAlign: 'center' }, compareExpanded: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#292D36' },
