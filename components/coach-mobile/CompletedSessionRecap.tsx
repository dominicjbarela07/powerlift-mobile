...
import { ActivityIndicator, Image, type ImageSourcePropType, Pressable, RefreshControl, StyleSheet, Switch, TouchableWithoutFeedback, View } from 'react-native';
...
import {
  ...
  type SessionRecapRelatedHistory,
  type SessionRecapRelatedReference,
} from '@/lib/session-recap-related-history';
import {
...
import { buildPostSessionTabs, type PostSessionTabKey } from '@/lib/post-session-shell';

export type CompletedRecapSet = Pick<CanonicalMovementArtworkInput, 'performed_canonical_movement_definition_id' | 'performed_canonical_movement_identity' | 'identity_snapshot' | 'primary_muscle_group_snapshot' | 'secondary_muscle_groups_snapshot'> & {
  id: number;
...
export type RecapTab = PostSessionTabKey;
...
function relatedReferenceSet(reference: SessionRecapRelatedReference): CompletedRecapSet | null {
  const set = reference.last_set;
  if (!set) return null;
  return {
    id: Number(set.set_log_id || 0),
    actual_weight_kg: set.weight_kg,
    actual_reps: set.reps,
    actual_rpe: set.rpe,
    actual_rir: set.rir,
...
function relatedReferenceResult(
  reference: SessionRecapRelatedReference,
  movement: CompletedRecapMovement,
  unit: DisplayWeightUnit,
) {
  const set = relatedReferenceSet(reference);
  if (!set) return 'Performance unavailable';
  const relatedMovement: CompletedRecapMovement = {
    ...movement,
    measurement: {
      ...movement.measurement,
      measurement_type: reference.measurement_type || movement.measurement?.measurement_type,
      load_convention: reference.load_convention,
      equipment_type: reference.equipment_type,
    },
  };
  const effort = effortLabel(set);
  return `${setResultLabel(set, relatedMovement, unit)}${effort ? ` · ${effort}` : ''}`;
}

function RelatedHistoryContextPanel({
  context,
  movement,
  currentSet,
  currentEquipment,
  ...
  context: SessionRecapRelatedHistory;
  movement: CompletedRecapMovement;
  currentSet: CompletedRecapSet | null;
  currentEquipment: string;
  unit: DisplayWeightUnit;
  expanded?: boolean;
}) {
  const references = (context.references || []).slice(0, expanded ? 5 : 2);
  return <View accessibilityLabel="Exact point with separate related-equipment history" style={styles.relatedContextPanel}>
    <View style={styles.relatedContextHeader}>
      <View style={styles.relatedContextHeaderCopy}>
        <Text style={styles.relatedContextKicker}>EXACT POINT + RELATED CONTEXT</Text>
        <Text style={styles.relatedContextMeta}>Separate equipment identities · no load delta</Text>
      </View>
      <View style={styles.contextOnlyBadge}><Text style={styles.contextOnlyBadgeText}>CONTEXT ONLY</Text></View>
    </View>
    <View style={styles.relatedExactRow}>
      <View style={[styles.contextMarker, styles.contextMarkerExact]} />
      <View style={styles.relatedContextCopy}>
        <Text style={styles.relatedContextLabel}>EXACT TODAY · FIRST EXPOSURE</Text>
        <Text numberOfLines={1} style={styles.relatedContextIdentity}>{currentEquipment}</Text>
      </View>
      <Text numberOfLines={1} style={styles.relatedContextResult}>{currentSet ? setResultLabel(currentSet, movement, unit) : 'Recorded'}</Text>
    </View>
    <Text style={styles.relatedDividerLabel}>RELATED HISTORY · DIFFERENT EQUIPMENT</Text>
    {references.map((reference) => <View key={`${reference.movement_definition_id}-${reference.last_set?.set_log_id}`} style={styles.relatedReferenceRow}>
      <View style={[styles.contextMarker, styles.contextMarkerRelated]} />
      <View style={styles.relatedContextCopy}>
        <Text numberOfLines={1} style={styles.relatedContextIdentity}>{relatedEquipmentIdentityLabel(reference)}</Text>
        <Text numberOfLines={1} style={styles.relatedContextDate}>{dateLabel(reference.last_performed_on || reference.last_set?.date, false)} · previous related performance</Text>
      </View>
      <Text numberOfLines={1} style={styles.relatedContextResult}>{relatedReferenceResult(reference, movement, unit)}</Text>
    </View>)}
    {expanded && (context.references?.length || 0) > references.length ? <Text style={styles.relatedContextMore}>{(context.references?.length || 0) - references.length} older related identit{(context.references?.length || 0) - references.length === 1 ? 'y' : 'ies'} available in Movement History</Text> : null}
    <Text style={styles.relatedContextDisclaimer}>Related performances are shown as recorded. Different equipment loads are not treated as equivalent.</Text>
  </View>;
}

function ExactFirstPerformancePanel({ expanded = false }: { expanded?: boolean }) {
  return <View style={[styles.exactBaselinePanel, expanded && styles.exactBaselinePanelExpanded]}>
    <Text style={styles.exactBaselineTitle}>FIRST PERFORMANCE</Text>
    <Text style={styles.exactBaselineBody}>Baseline established. Future performances on this exact equipment will build progression history.</Text>
  </View>;
}
...
function PerformedMovementCard({ movement, analysis, unit, onVideo, onOpenHistory, initialExpanded = false }: { movement: CompletedRecapMovement; analysis?: ReviewerMovementEvidence | null; unit: DisplayWeightUnit; onVideo: (set: CompletedRecapSet) => void; onOpenHistory?: (movement: CompletedRecapMovement) => void; initialExpanded?: boolean }) {
  ...
  const currentEquipment = equipment[0]?.label || (equipment[0] ? equipmentSecondaryLabel(equipment[0]) : 'Exact equipment identity');
        ...
        <View style={styles.movementSummary}>
          ...
          <View style={styles.movementComparisonGrid}>
            <View style={styles.movementComparisonPair}>
              ...
              <View style={styles.movementComparisonCell}><Text style={styles.movementComparisonLabel}>{relatedContext ? 'EXACT HISTORY' : 'LAST TIME'}</Text><Text numberOfLines={2} style={styles.movementComparisonValue}>{previousAsSet ? `${setResultLabel(previousAsSet, movement, unit)}${effortLabel(previousAsSet) ? ` · ${effortLabel(previousAsSet)}` : ''}` : relatedContext ? `First exposure · ${currentEquipment}` : 'No prior comparable Session'}</Text></View>
            </View>
...
function formatDurationComparison(analytics: ReviewerAnalytics, recap: CompletedSessionRecapPayload) {
  ...
  if (current == null) return { value: 'Started', detail: 'Duration baseline started' };
  ...
  if (baseline == null || deltaSeconds == null) return { value: durationLabel(current) || 'Duration baseline started', detail: 'Duration baseline started' };
...
function sessionConclusion(counts: Record<string, number>) {
  const improved = Number(counts.improved || 0);
  const stable = Number(counts.stable || 0);
  const declined = Number(counts.declined || 0);
  const comparable = improved + stable + declined;
  if (!comparable) return 'Baseline Building';
  if (improved > 0 && declined === 0) return 'Good Session';
  if (declined > improved) return 'Session Needs Attention';
  return 'Productive Session';
}

function comparisonConclusion(analytics: ReviewerAnalytics) {
  const volumeDelta = analytics.what_changed?.volume?.delta_percent;
  const effortDelta = analytics.what_changed?.average_effort_rpe_equivalent?.delta;
  if (!analytics.comparator) return 'Baseline Building';
  const work = volumeDelta == null || Math.abs(volumeDelta) < 5 ? 'Similar Work' : volumeDelta > 0 ? 'More Work' : 'Less Work';
  const effort = effortDelta == null || Math.abs(effortDelta) <= 0.5 ? 'Similar Effort' : effortDelta > 0 ? 'Higher Effort' : 'Lower Effort';
  return `${work}, ${effort}`;
}

function sessionOverviewNarrative(analytics: ReviewerAnalytics) {
  if (!analytics.comparator) return 'Baseline building. This Session is recorded, and comparable performance context will appear as matching history builds.';
  const volumeDelta = analytics.what_changed?.volume?.delta_percent;
  const effortDelta = analytics.what_changed?.average_effort_rpe_equivalent?.delta;
  const recoveryState = String(analytics.recovery?.state || analytics.session_read?.recovery?.state || '');
  const work = volumeDelta == null || Math.abs(volumeDelta) < 5
    ? 'Similar total work to your last comparable Session'
    : volumeDelta > 0
      ? 'More total work than your last comparable Session'
      : 'Less total work than your last comparable Session';
  const effort = effortDelta == null || Math.abs(effortDelta) <= 0.5
    ? 'with similar effort.'
    : effortDelta < 0
      ? 'with lower effort.'
      : 'with higher effort.';
  const recovery = recoveryState.includes('below')
    ? ' Recovery was below your normal range.'
    : recoveryState
      ? ' Recovery was within your normal range.'
      : '';
  return `${work} ${effort}${recovery}`;
}

function HeroEvidenceRow({ icon, value, label, tone }: { icon: React.ComponentProps<typeof Ionicons>['name']; value: string; label: string; tone: string }) {
  return <View style={styles.overviewHeroEvidenceRow}><View style={[styles.overviewHeroEvidenceIcon, { backgroundColor: `${tone}18` }]}><Ionicons name={icon} size={26} color={tone} /></View><View style={styles.overviewHeroEvidenceCopy}><Text style={styles.overviewHeroEvidenceValue}>{value}</Text><Text style={styles.overviewHeroEvidenceLabel}>{label}</Text></View></View>;
}

function SessionReadOverview({ analytics, recap, personalBestCount }: { analytics: ReviewerAnalytics; recap: CompletedSessionRecapPayload; personalBestCount: number }) {
  ...
  return <View style={styles.overviewSessionHero} testID="post-session-overview-result-hero">
    <LinearGradient colors={['rgba(87,35,138,0.30)', 'rgba(15,9,24,0.94)', '#080710']} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
    <View style={styles.overviewSessionHeroMain}><View style={styles.overviewSessionHeroLead}><View style={styles.overviewSessionHeroIcon}><Ionicons name="analytics-outline" size={34} color="#BE99EF" /></View><View style={styles.overviewSessionHeroCopy}><Text style={styles.overviewKicker}>SESSION RESULT</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.88} style={styles.overviewMajorConclusion}>{sessionConclusion(counts)}</Text></View></View><Text style={styles.overviewOutcomeSummary}>{counts.improved || 0} improved · {counts.stable || 0} stable · {counts.declined || 0} declined</Text><Text style={styles.overviewNarrative}>{sessionOverviewNarrative(analytics)}</Text></View>
    <View style={styles.overviewHeroEvidence}><HeroEvidenceRow icon="time-outline" value={duration.value} label={duration.value === 'Started' ? duration.detail : 'Duration'} tone="#9FB9FF" /><HeroEvidenceRow icon="barbell-outline" value={String(recap.session.set_count)} label="Sets logged" tone="#B9C8FF" />{personalBestCount > 0 ? <HeroEvidenceRow icon="trophy-outline" value={String(personalBestCount)} label={`New PR${personalBestCount === 1 ? '' : 's'}`} tone="#F0AD28" /> : null}</View>
  </View>;
}

function ComparisonMetricCard({ icon, value, label, detail, tone }: { icon: React.ComponentProps<typeof Ionicons>['name']; value: string; label: string; detail: string; tone: string }) {
  return <View style={styles.overviewComparisonMetric}><View style={[styles.overviewComparisonIcon, { backgroundColor: `${tone}18` }]}><Ionicons name={icon} size={27} color={tone} /></View><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.6} style={[styles.overviewComparisonValue, { color: tone }]}>{value}</Text><Text style={styles.overviewComparisonLabel}>{label}</Text><Text style={styles.overviewComparisonDetail}>{detail}</Text></View>;
}

function OutcomeCard({ icon, count, label, tone, active }: { icon: React.ComponentProps<typeof Ionicons>['name']; count: number; label: string; tone: string; active?: boolean }) {
  return <View style={[styles.overviewOutcomeCard, active && { borderColor: tone, backgroundColor: `${tone}0E` }]}><View style={[styles.overviewOutcomeIcon, { backgroundColor: `${tone}18` }]}><Ionicons name={icon} size={25} color={tone} /></View><View><Text style={[styles.overviewOutcomeCount, { color: tone }]}>{count}</Text><Text style={styles.overviewOutcomeLabel}>{label}</Text></View></View>;
}

function WhatChangedOverview({ analytics, recap, movements, unit, onOpenMovementHistory }: { analytics: ReviewerAnalytics; recap: CompletedSessionRecapPayload; movements: CompletedRecapMovement[]; unit: DisplayWeightUnit; onOpenMovementHistory?: (movement: CompletedRecapMovement) => void }) {
  ...
  const outcomes = changed.movement_outcomes || analytics.session_read?.performance?.counts || {};
  const currentVolume = formatCompactVolumeValueFromKg(volume.current_kg ?? recap.session.total_volume_kg, unit) || 'Baseline';
  const previousVolume = formatCompactVolumeValueFromKg(volume.previous_kg, unit);
  const improvedRows = (analytics.movements || []).filter((row) => row.comparison?.state === 'improved');
  const strongestAnalysis = [...improvedRows].sort((left, right) => Math.abs(Number(right.comparison?.metric_delta_percent || 0)) - Math.abs(Number(left.comparison?.metric_delta_percent || 0)))[0];
  const strongestMovement = movements.find((movement) => Number(movement.item_id) === Number(strongestAnalysis?.item_id));
  const strongestComparison = strongestMovement && strongestAnalysis
    ? formatMovementPerformanceComparison(compareMovementPerformance(
      recapSetPerformance(strongestMovement.best_set),
      recapSetPerformance(strongestAnalysis.previous_best),
      recapMovementLoadSemantics(strongestMovement),
    ), unit)
    : null;
  const strongestDelta = strongestAnalysis?.comparison?.metric_delta_percent == null
    ? strongestComparison || strongestAnalysis?.comparison?.literal || 'Improved best-set performance'
    : `${signed(strongestAnalysis.comparison.metric_delta_percent, '%') || 'Improved'} best-set performance`;
  const comparableCount = Number(outcomes.improved || 0) + Number(outcomes.stable || 0) + Number(outcomes.declined || 0);
  const effortTone = Number(effort.delta || 0) <= 0 ? '#00DF88' : '#F0AD28';
  return <View style={styles.overviewComparisonCard} testID="post-session-overview-comparison-card">
    <LinearGradient colors={['rgba(21,35,58,0.28)', 'rgba(4,8,14,0.96)']} style={StyleSheet.absoluteFillObject} />
    <View style={styles.overviewComparisonHeader}><View style={styles.overviewComparisonHeaderCopy}><Text style={styles.overviewKicker}>{analytics.comparator?.label ? `SINCE ${analytics.comparator.label.toUpperCase()}` : 'LAST COMPARABLE SESSION'}</Text><Text style={styles.overviewSectionConclusion}>{comparisonConclusion(analytics)}</Text><Text style={styles.overviewSectionSupport}>{analytics.comparator ? 'Compared to your last comparable Session.' : 'Baseline building. A matching Session will unlock change evidence.'}</Text></View><Ionicons name="chevron-forward" size={30} color="#B8C7FF" /></View>
    <View style={styles.overviewComparisonMetrics}>
      <ComparisonMetricCard icon="bar-chart-outline" value={volume.previous_kg == null ? 'Baseline' : signed(volume.delta_percent, '%', 0) || '±0%'} label="Total volume" detail={previousVolume ? `${currentVolume} vs ${previousVolume}` : `${currentVolume} · Baseline building`} tone="#00DF88" />
      <ComparisonMetricCard icon="layers-outline" value={sets.previous == null ? 'Baseline' : `${Number(sets.delta || 0) > 0 ? '+' : Number(sets.delta || 0) < 0 ? '−' : '±'}${Math.abs(Number(sets.delta || 0))}`} label="Sets completed" detail={sets.previous == null ? `${sets.current ?? recap.session.set_count} · Baseline building` : `${sets.current ?? recap.session.set_count} vs ${sets.previous}`} tone="#00DF88" />
      <ComparisonMetricCard icon="flash-outline" value={effort.previous == null ? 'Baseline' : signed(effort.delta) || '±0.0'} label="Average effort" detail={effort.previous == null ? (effort.current == null ? 'Baseline building' : `${numberLabel(effort.current)} RPE-eq · Baseline building`) : `${numberLabel(effort.current)} vs ${numberLabel(effort.previous)} RPE-eq`} tone={effortTone} />
    </View>
    <View style={styles.overviewMovementOutcomesHeader}><Text style={styles.overviewSubsectionKicker}>MOVEMENT OUTCOMES</Text><Text style={styles.overviewComparableCount}>{comparableCount ? `${comparableCount} comparable movement${comparableCount === 1 ? '' : 's'}` : 'Baseline building'}</Text></View>
    <View style={styles.overviewOutcomeStrip}><OutcomeCard icon="arrow-up" count={Number(outcomes.improved || 0)} label="Improved" tone="#00E990" active={Number(outcomes.improved || 0) > 0} /><OutcomeCard icon="remove" count={Number(outcomes.stable || 0)} label="Stable" tone="#B8C7FF" active={Number(outcomes.stable || 0) > 0} /><OutcomeCard icon="arrow-down" count={Number(outcomes.declined || 0)} label="Declined" tone="#FF5964" active={Number(outcomes.declined || 0) > 0} /></View>
    {strongestMovement ? <Pressable accessibilityRole="button" accessibilityLabel={`Open ${strongestMovement.label} movement history`} disabled={!onOpenMovementHistory} onPress={() => onOpenMovementHistory?.(strongestMovement)} style={({ pressed }) => [styles.overviewProgressionRow, pressed && styles.pressed]}><View style={styles.overviewProgressionArtwork}><CanonicalMovementArtwork movement={strongestMovement} size={74} testID="post-session-overview-biggest-progression-artwork" /></View><View style={styles.overviewProgressionCopy}><Text style={styles.overviewProgressionKicker}>Biggest progression</Text><Text numberOfLines={2} style={styles.overviewProgressionMovement}>{strongestMovement.label}</Text><Text style={styles.overviewProgressionDetail}>{strongestDelta}</Text></View>{onOpenMovementHistory ? <Ionicons name="chevron-forward" size={30} color="#C0CBFF" /> : null}</Pressable> : <View style={styles.overviewBaselineRow}><Ionicons name="analytics-outline" size={27} color="#B8C7FF" /><View style={styles.overviewBaselineCopy}><Text style={styles.overviewBaselineTitle}>Baseline building</Text><Text style={styles.overviewBaselineBody}>A comparable movement will appear here after its next matching performance.</Text></View></View>}
  </View>;
}
...
function RecoveryOverview({ analytics, recap }: { analytics: ReviewerAnalytics; recap: CompletedSessionRecapPayload }) {
  ...
  const selected: RecoveryMetricKey = available.includes('readiness') ? 'readiness' : available[0] || 'readiness';
  const presentation = RECOVERY_PRESENTATION[selected];
  const selectedMetric = hydrated[selected];
  const historicalPoints = (recovery.trend || []).flatMap((row, index) => {
  ...
  const hasCurrentPoint = historicalPoints.some((point) => Boolean((point.meta as Record<string, any>)?.current));
  const points = selectedMetric.value != null && !hasCurrentPoint
    ? [...historicalPoints, { id: `current:${recap.session.date || 'Session'}`, date: String(recap.session.date || new Date().toISOString().slice(0, 10)), value: Number(selectedMetric.value), meta: { current: true } }]
    : historicalPoints;
  ...
  const statusLabel = recovery.label || (available.length ? 'Typical' : 'Baseline building');
  const recoveryConclusion = !available.length
    ? 'Baseline building'
    : String(recovery.state || '').includes('below')
      ? 'Below normal heading into this Session'
      : String(recovery.state || '').includes('above')
        ? 'Above normal heading into this Session'
        : 'Near normal heading into this Session';
  return <View style={styles.overviewRecoveryCard} testID="post-session-overview-recovery-card"><LinearGradient colors={['rgba(70,29,108,0.28)', 'rgba(6,8,15,0.97)']} style={StyleSheet.absoluteFillObject} /><View style={styles.overviewRecoveryHeader}><View style={styles.overviewRecoveryHeaderCopy}><Text style={styles.overviewRecoveryKicker}>RECOVERY CONTEXT</Text><Text style={styles.overviewSectionConclusion}>{recoveryConclusion}</Text></View><View style={styles.overviewRecoveryPill}><Text style={styles.overviewRecoveryPillText}>{statusLabel}</Text></View></View>{available.length ? <><View style={styles.overviewRecoveryMetrics}>{available.map((key, index) => {
    const item = hydrated[key];
    const row = RECOVERY_PRESENTATION[key];
    const deltaValue = item.delta == null ? null : Number(item.delta);
    const delta = deltaValue == null ? 'Baseline building' : `${Math.abs(deltaValue) < 0.05 ? '±' : deltaValue > 0 ? '+' : '−'}${Math.abs(deltaValue).toFixed(1)}`;
    return <View key={key} style={[styles.overviewRecoveryMetric, index < 3 ? styles.overviewRecoveryMetricThird : styles.overviewRecoveryMetricHalf]}><View style={[styles.overviewRecoveryMetricIcon, { backgroundColor: `${row.color}18` }]}><Ionicons name={row.icon} size={27} color={row.color} /></View><View style={styles.overviewRecoveryMetricCopy}><Text style={[styles.overviewRecoveryMetricValue, { color: key === 'sleep' ? row.color : SLColors.textPrimary }]}>{item.value == null ? 'Baseline' : `${numberLabel(item.value)}${key === 'sleep' ? 'h' : ''}`}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.9} style={styles.overviewRecoveryMetricLabel}>{row.label}</Text><Text style={styles.overviewRecoveryMetricDetail}>{item.baseline == null ? 'Baseline building' : `vs ${numberLabel(item.baseline)}${key === 'sleep' ? 'h' : ''}  (${delta})`}</Text></View></View>;
  })}</View><View style={styles.overviewRecoveryChart}><View style={styles.overviewRecoveryChartHeader}><View style={styles.overviewRecoveryChartTitleRow}><View style={styles.overviewRecoveryChartIcon}><Ionicons name="trending-up-outline" size={24} color="#B8C7FF" /></View><Text style={styles.overviewRecoveryChartTitle}>{presentation.label} Trend</Text></View><Text style={styles.overviewRecoveryChartRange}>Last 30 days</Text></View><AnalyticalTimeSeriesChart band={band} bandLabel={band.length ? 'Recent baseline' : undefined} emptyTitle={`${presentation.label} baseline building`} emptyBody={`The current ${presentation.label.toLowerCase()} value is recorded. Another dated observation will establish a trend.`} formatSeriesValue={(_key, value) => `${numberLabel(value)}${presentation.suffix}`} height={270} largeReadableText metric={analyticalMetricDefinition(`post_session_${selected}`, { label: presentation.label, kind: selected === 'sleep' ? 'hours' : 'score', unit: selected === 'sleep' ? 'h' : '/10', axisUnit: selected === 'sleep' ? 'h' : undefined, minimum: 0, maximum: presentation.maximum, maximumFractionDigits: 1 })} readableText selectedInitially="latest" series={[{ key: selected, label: presentation.label, color: presentation.color, points }]} showLegend={false} testID="canonical-post-session-recovery-chart" tooltipRows={(selection) => [((selection.values[0]?.meta as any)?.current || selection.date.slice(0, 10) === recap.session.date?.slice(0, 10)) ? 'This Session' : 'Prior readiness observation', selectedMetric.baseline != null ? `Recent baseline ${numberLabel(selectedMetric.baseline)}${presentation.suffix}` : 'Baseline building']} /></View></> : <View style={styles.overviewBaselineRow}><Ionicons name="moon-outline" size={27} color="#B8C7FF" /><View style={styles.overviewBaselineCopy}><Text style={styles.overviewBaselineTitle}>Baseline building</Text><Text style={styles.overviewBaselineBody}>Readiness, sleep, stress, energy, and soreness context will appear after a pre-Session check-in.</Text></View></View>}</View>;
}

function ReflectionOverview({ analytics, recap, onEditReflection }: { analytics: ReviewerAnalytics; recap: CompletedSessionRecapPayload; onEditReflection?: () => void }) {
  ...
  const reflectionLabel = strength ? humanize(strength) : reflection.label || humanize(fatigue.value);
  const reflectionBody = note || (reflection.sample_size ? 'Aligned with your recent Sessions.' : 'Athlete-reported context for this Session.');
  return <><View style={styles.overviewReflectionCard} testID="post-session-overview-reflection-card"><LinearGradient colors={['rgba(81,25,110,0.35)', 'rgba(11,8,19,0.96)']} style={StyleSheet.absoluteFillObject} /><View style={styles.overviewReflectionIcon}><Ionicons name="chatbubble-ellipses-outline" size={32} color="#DE62FF" /></View><View style={styles.overviewReflectionCopy}><Text style={styles.overviewReflectionKicker}>ATHLETE REFLECTION</Text>{hasReflection ? <><Text style={styles.overviewReflectionValue}>{reflectionLabel}</Text><Text numberOfLines={3} style={styles.overviewReflectionDetail}>{reflectionBody}</Text></> : <><Text style={styles.overviewReflectionValue}>Baseline building</Text><Text style={styles.overviewReflectionDetail}>Reflection context will appear after a post-Session check-in.</Text></>}</View></View>{onEditReflection ? <Pressable accessibilityRole="button" onPress={onEditReflection} style={({ pressed }) => [styles.overviewReflectionEdit, pressed && styles.pressed]}><Text style={styles.overviewReflectionEditText}>{hasReflection ? 'Edit reflection' : 'Add reflection'}</Text><Ionicons name="arrow-forward" size={18} color={SLColors.accentMuted} /></Pressable> : null}</>;
}
...
function PostSessionToolsSheet({ visible, movements, tabs, activeTab, onSelectTab, onClose, onResumeSession, onCorrectEquipment, onEditSetEvidence, onEditSessionNotes, onOpenMovementHistory, onViewSessionHistory }: { visible: boolean; movements: CompletedRecapMovement[]; tabs: { key: RecapTab; label: string }[]; activeTab: RecapTab; onSelectTab: (tab: RecapTab) => void; onClose: () => void; onResumeSession?: () => void; onCorrectEquipment?: (movement: CompletedRecapMovement) => void; onEditSetEvidence?: () => void; onEditSessionNotes?: () => void; onOpenMovementHistory?: (movement: CompletedRecapMovement) => void; onViewSessionHistory?: () => void }) {
...
export function CompletedSessionRecap({ recap, onEditReflection, impactSummary, preferredUnits, refreshing, onRefresh, onClose, onDone, initialTab = 'overview', initialToolsOpen = false, initialScrollOffsetY = 0, initialExpandedItemId, viewerMode = 'athlete', sessionTimeZone, parentProvidesTopSafeArea = false, coachReview, coachReviewUnavailableReason, onViewLedger, onViewCalendar, onLogNextSession, onOpenProgramming, onOpenMovementHistory, onResumeSession, onCorrectEquipment, onEditSetEvidence, onEditSessionNotes, onViewSessionHistory }: Props) {
  ...
  const visibleTabs = buildPostSessionTabs({
    viewerMode,
    hasPersonalBests: personalBestEvidence.length > 0,
  });
  ...
  const hasPostSessionMenu = visibleTabs.length > 1 || hasAthleteTools;
  ...
  return <SafeAreaView edges={parentProvidesTopSafeArea ? [] : ['top']} style={styles.screen}>
    <FloatingControlCoordinator context="screen">
      ...
      <ScrollView ref={contentScrollRef} contentOffset={{ x: 0, y: initialScrollOffsetY }} contentContainerStyle={[styles.canonicalContent, { paddingBottom: Math.max(insets.bottom, 18) + 94 }]} refreshControl={onRefresh ? <RefreshControl refreshing={!!refreshing} onRefresh={onRefresh} tintColor={SLColors.accent} /> : undefined} showsVerticalScrollIndicator={false}>
        ...
        {deepActions.length ? <View style={[styles.nextActions, tab === 'overview' && styles.overviewNextActions]}>{deepActions.map((action) => <ActionButton key={action.label} {...action} />)}</View> : null}
      </ScrollView>
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#050409' }, pressed: { opacity: 0.78, transform: [{ scale: 0.985 }] }, content: { gap: 13 }, sectionShell: {}, edgeToEdge: { marginHorizontal: 0, paddingHorizontal: 0 },
  topBar: { flexDirection: 'row', alignItems: 'center', minHeight: 78, paddingHorizontal: 12, paddingBottom: 8, backgroundColor: '#050409' }, topButton: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderRadius: 13, borderWidth: 1, borderColor: '#30243E', backgroundColor: '#100D17' }, topBarCopy: { flex: 1, minWidth: 0, alignItems: 'center', paddingHorizontal: 8 }, topTitle: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 18, lineHeight: 21, textAlign: 'center' }, topDot: { color: SLColors.accentMuted }, topSubtitle: { marginTop: 2, color: SLColors.textSecondary, fontFamily: SLFontFamilies.body, fontSize: 16 }, completeMark: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderRadius: 13, borderWidth: 1, borderColor: '#30243E', backgroundColor: '#100D17' },
  hero: { position: 'relative', overflow: 'hidden', minHeight: 286, padding: 16, paddingBottom: 0, borderRadius: SLRadius.xl, borderWidth: 1, borderColor: '#60358A', backgroundColor: '#07080E', ...SLShadows.level2 }, heroAtmosphere: { position: 'absolute', top: 0, right: 0, bottom: 70, width: '58%' }, heroAnatomy: { position: 'absolute', top: 14, right: 12, width: 126, height: 152, alignItems: 'center', justifyContent: 'center', opacity: 1 }, heroFallbackArt: { width: 220, height: 220, opacity: 0.72 }, heroCopyShade: { position: 'absolute', zIndex: 1, top: 0, bottom: 70, left: 0, width: '78%' }, heroCopy: { zIndex: 2, width: '58%', minHeight: 195, paddingTop: 3 }, heroKicker: { color: SLColors.success, fontFamily: SLFontFamilies.bodyBold, fontSize: 9, letterSpacing: 1.2 }, heroTitle: { marginTop: 9, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 29, lineHeight: 32 }, heroIdentity: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 12 }, heroIdentityCopy: { flex: 1, minWidth: 0 }, athleteInitials: { width: 34, height: 34, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', borderRadius: 17, borderWidth: 1, borderColor: 'rgba(182,112,255,0.55)', backgroundColor: '#1A1024' }, athleteAvatar: { width: '100%', height: '100%' }, athleteInitialsText: { color: SLColors.accentMuted, fontFamily: SLFontFamilies.bodyBold, fontSize: 10 }, heroAthlete: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 13 }, heroMeta: { marginTop: 3, color: SLColors.textSecondary, fontFamily: SLFontFamilies.body, fontSize: 10 }, notesPill: { position: 'absolute', zIndex: 3, top: 146, left: 16, flexDirection: 'row', alignItems: 'center', gap: 6, minHeight: 34, paddingHorizontal: 10, borderRadius: 10, borderWidth: 1, borderColor: SLColors.borderStandard, backgroundColor: 'rgba(8,10,16,0.88)' }, notesPillText: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 9 }, summaryMetricRow: { zIndex: 4, flexDirection: 'row', minHeight: 72, marginHorizontal: -16, paddingHorizontal: 7, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(174,104,255,0.28)', backgroundColor: 'rgba(4,5,9,0.94)' }, summaryMetric: { width: '33.333%', minWidth: 0, minHeight: 62, flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, borderRightWidth: StyleSheet.hairlineWidth, borderRightColor: '#241C2D', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#241C2D' }, summaryMetricCopy: { flex: 1, minWidth: 0 }, summaryMetricValue: { color: '#F1EAF8', fontFamily: SLFontFamilies.display, fontSize: 19 }, summaryMetricLabel: { marginTop: 3, color: '#ACA2B7', fontFamily: SLFontFamilies.body, fontSize: 11.5 },
  ...
  focusCard: { position: 'relative', flexDirection: 'row', minHeight: 245, overflow: 'hidden', marginTop: 8, padding: 12, borderRadius: SLRadius.xl, borderWidth: 1, borderColor: '#39244F', backgroundColor: '#05060A' }, focusChart: { width: 162, height: 222, alignItems: 'center', justifyContent: 'center' }, focusAnatomy: { width: 94, height: 128 }, focusBreakdown: { flex: 1, justifyContent: 'center', gap: 9, marginLeft: -8 }, focusSummary: { marginBottom: 2, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 13 }, focusRow: { gap: 4 }, focusRowTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 6 }, focusName: { flex: 1, color: SLColors.textSecondary, fontFamily: SLFontFamilies.bodyBold, fontSize: 9 }, focusRank: { color: SLColors.accentMuted, fontFamily: SLFontFamilies.bodyBold, fontSize: 6.2 }, focusTrack: { height: 5, overflow: 'hidden', borderRadius: 3, backgroundColor: SLColors.surfaceInset }, focusFill: { height: 5, borderRadius: 3 }, evidenceSource: { marginTop: 5, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 7, lineHeight: 10 },
  ...
  relatedContextPanel: { marginTop: 9, overflow: 'hidden', borderRadius: 11, borderWidth: 1, borderColor: '#315267', backgroundColor: '#050A0F' },
  relatedContextHeader: { minHeight: 45, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 9, paddingVertical: 7, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#29404E' },
  relatedContextHeaderCopy: { flex: 1, minWidth: 0 }, relatedContextKicker: { color: '#72D8F0', fontFamily: SLFontFamilies.bodyBold, fontSize: 9, letterSpacing: 0.65 }, relatedContextMeta: { marginTop: 3, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 9 },
  contextOnlyBadge: { paddingHorizontal: 6, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: '#315D70', backgroundColor: 'rgba(67,179,207,0.1)' }, contextOnlyBadgeText: { color: '#72D8F0', fontFamily: SLFontFamilies.bodyBold, fontSize: 7, letterSpacing: 0.45 },
  relatedExactRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 9, paddingVertical: 7, backgroundColor: 'rgba(135,72,191,0.08)' }, relatedReferenceRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 9, paddingVertical: 7, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#22323B' },
  contextMarker: { width: 8, height: 8, borderRadius: 4 }, contextMarkerExact: { backgroundColor: '#B36BFF' }, contextMarkerRelated: { backgroundColor: '#53CBE8' }, relatedContextCopy: { flex: 1, minWidth: 0 }, relatedContextLabel: { color: '#CE9BFF', fontFamily: SLFontFamilies.bodyBold, fontSize: 8, letterSpacing: 0.45 }, relatedContextIdentity: { marginTop: 2, color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 10 }, relatedContextDate: { marginTop: 3, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 8.5 }, relatedContextResult: { maxWidth: '34%', color: SLColors.textSecondary, fontFamily: SLFontFamilies.bodyBold, fontSize: 9.5, textAlign: 'right' }, relatedDividerLabel: { paddingHorizontal: 9, paddingTop: 8, paddingBottom: 4, color: '#72D8F0', fontFamily: SLFontFamilies.bodyBold, fontSize: 7.5, letterSpacing: 0.55 }, relatedContextMore: { paddingHorizontal: 9, paddingVertical: 7, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 8.5 }, relatedContextDisclaimer: { paddingHorizontal: 9, paddingVertical: 7, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#29404E', color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 8.5, lineHeight: 12 },
  exactBaselinePanel: { marginTop: 9, padding: 10, borderRadius: 11, borderWidth: StyleSheet.hairlineWidth, borderColor: '#3A3541', backgroundColor: '#07080D' }, exactBaselinePanelExpanded: { margin: 10, padding: 12 }, exactBaselineTitle: { color: '#C999ED', fontFamily: SLFontFamilies.bodyBold, fontSize: 9, letterSpacing: 0.7 }, exactBaselineBody: { marginTop: 4, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 9.5, lineHeight: 14 },
  ...
  canonicalContent: { gap: 14 },
  overviewSessionHero: { position: 'relative', minHeight: 296, overflow: 'hidden', flexDirection: 'row', borderWidth: 1, borderColor: '#4B315F', backgroundColor: '#0A0811' },
  overviewSessionHeroMain: { flex: 1, minWidth: 0, paddingHorizontal: 12, paddingVertical: 17 },
  overviewSessionHeroLead: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  overviewSessionHeroIcon: { width: 56, height: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 17, borderWidth: 1, borderColor: 'rgba(182,123,242,0.30)', backgroundColor: 'rgba(93,44,145,0.25)' },
  overviewSessionHeroCopy: { flex: 1, minWidth: 0 },
  overviewKicker: { color: '#C8D5FF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16, lineHeight: 21, letterSpacing: 1.4 },
  overviewMajorConclusion: { marginTop: 3, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 32, lineHeight: 36 },
  overviewOutcomeSummary: { marginTop: 12, color: '#DEE5FF', fontFamily: SLFontFamilies.bodyBold, fontSize: 17, lineHeight: 23 },
  overviewNarrative: { marginTop: 8, color: '#C3CEE8', fontFamily: SLFontFamilies.body, fontSize: 18, lineHeight: 26 },
  overviewHeroEvidence: { width: '34%', minWidth: 128, justifyContent: 'center', gap: 16, marginVertical: 16, paddingHorizontal: 5, borderLeftWidth: 1, borderLeftColor: '#304052' },
  overviewHeroEvidenceRow: { minHeight: 58, flexDirection: 'row', alignItems: 'center', gap: 6 },
  overviewHeroEvidenceIcon: { width: 34, height: 34, alignItems: 'center', justifyContent: 'center', borderRadius: 10 },
  overviewHeroEvidenceCopy: { flex: 1, minWidth: 0 },
  overviewHeroEvidenceValue: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 22, lineHeight: 26 },
  overviewHeroEvidenceLabel: { marginTop: 2, color: '#C3CEE8', fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 20 },
  overviewComparisonCard: { position: 'relative', overflow: 'hidden', borderWidth: 1, borderColor: '#33445A', backgroundColor: '#05090F' },
  overviewComparisonHeader: { minHeight: 135, flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 20, paddingVertical: 17 },
  overviewComparisonHeaderCopy: { flex: 1, minWidth: 0 },
  overviewSectionConclusion: { marginTop: 4, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 28, lineHeight: 34 },
  overviewSectionSupport: { marginTop: 4, color: '#C3CEE8', fontFamily: SLFontFamilies.body, fontSize: 18, lineHeight: 24 },
  overviewComparisonMetrics: { flexDirection: 'row', paddingHorizontal: 10, paddingBottom: 12 },
  overviewComparisonMetric: { width: '33.333%', minHeight: 170, paddingHorizontal: 9, paddingVertical: 12, borderWidth: 1, borderColor: '#26364A', backgroundColor: 'rgba(8,15,25,0.82)' },
  overviewComparisonIcon: { width: 45, height: 45, alignItems: 'center', justifyContent: 'center', borderRadius: 13 },
  overviewComparisonValue: { marginTop: 9, fontFamily: SLFontFamilies.display, fontSize: 27, lineHeight: 31 },
  overviewComparisonLabel: { marginTop: 4, color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 18, lineHeight: 22 },
  overviewComparisonDetail: { marginTop: 6, color: '#BBC7E0', fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 21 },
  overviewMovementOutcomesHeader: { minHeight: 49, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, paddingHorizontal: 20, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#33445A' },
  overviewSubsectionKicker: { color: '#C8D5FF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16, letterSpacing: 1.2 },
  overviewComparableCount: { flex: 1, color: '#BFCBE3', fontFamily: SLFontFamilies.body, fontSize: 16, textAlign: 'right' },
  overviewOutcomeStrip: { flexDirection: 'row', paddingHorizontal: 10, paddingBottom: 12 },
  overviewOutcomeCard: { width: '33.333%', minHeight: 94, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingHorizontal: 2, borderWidth: 1, borderColor: '#2A394B', backgroundColor: 'rgba(8,14,23,0.78)' },
  overviewOutcomeIcon: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 10 },
  overviewOutcomeCount: { fontFamily: SLFontFamilies.display, fontSize: 24, lineHeight: 27 },
  overviewOutcomeLabel: { marginTop: 2, color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 18, lineHeight: 21 },
  overviewProgressionRow: { minHeight: 116, flexDirection: 'row', alignItems: 'center', gap: 14, marginHorizontal: 10, marginBottom: 12, padding: 10, borderWidth: 1, borderColor: '#26364A', backgroundColor: 'rgba(4,11,17,0.9)' },
  overviewProgressionArtwork: { width: 84, height: 84, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', borderRadius: 14, borderWidth: 1, borderColor: '#344155', backgroundColor: '#090C13' },
  overviewProgressionCopy: { flex: 1, minWidth: 0 },
  overviewProgressionKicker: { color: '#00E990', fontFamily: SLFontFamilies.bodyBold, fontSize: 17, lineHeight: 21 },
  overviewProgressionMovement: { marginTop: 3, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 25, lineHeight: 29 },
  overviewProgressionDetail: { marginTop: 4, color: '#C7D3EB', fontFamily: SLFontFamilies.body, fontSize: 18, lineHeight: 24 },
  overviewBaselineRow: { minHeight: 100, flexDirection: 'row', alignItems: 'center', gap: 13, margin: 12, padding: 16, borderWidth: 1, borderColor: '#304057', backgroundColor: 'rgba(9,15,24,0.84)' },
  overviewBaselineCopy: { flex: 1, minWidth: 0 },
  overviewBaselineTitle: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 18, lineHeight: 23 },
  overviewBaselineBody: { marginTop: 4, color: '#BFCBE3', fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 22 },
  overviewRecoveryCard: { position: 'relative', overflow: 'hidden', borderWidth: 1, borderColor: '#593378', backgroundColor: '#080811', paddingBottom: 12 },
  overviewRecoveryHeader: { minHeight: 104, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, paddingHorizontal: 20, paddingVertical: 16 },
  overviewRecoveryHeaderCopy: { flex: 1, minWidth: 0 },
  overviewRecoveryKicker: { color: '#D16CFF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16, lineHeight: 21, letterSpacing: 1.3 },
  overviewRecoveryPill: { minHeight: 42, justifyContent: 'center', paddingHorizontal: 16, borderRadius: 21, borderWidth: 1, borderColor: '#A84CDE', backgroundColor: 'rgba(113,34,148,0.18)' },
  overviewRecoveryPillText: { color: '#DB91FF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16 },
  overviewRecoveryMetrics: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 10 },
  overviewRecoveryMetric: { minHeight: 130, flexDirection: 'row', alignItems: 'flex-start', gap: 3, paddingHorizontal: 3, paddingVertical: 13, borderWidth: 1, borderColor: '#283249', backgroundColor: 'rgba(9,14,25,0.74)' },
  overviewRecoveryMetricThird: { width: '33.333%' },
  overviewRecoveryMetricHalf: { width: '50%' },
  overviewRecoveryMetricIcon: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', borderRadius: 9 },
  overviewRecoveryMetricCopy: { flex: 1, minWidth: 0 },
  overviewRecoveryMetricValue: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 23, lineHeight: 27 },
  overviewRecoveryMetricLabel: { marginTop: 2, color: SLColors.textPrimary, fontFamily: SLFontFamilies.body, fontSize: 18, lineHeight: 22 },
  overviewRecoveryMetricDetail: { marginTop: 2, color: '#BBC7E0', fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 21 },
  overviewRecoveryChart: { marginHorizontal: 10, marginTop: 12, overflow: 'hidden', borderWidth: 1, borderColor: '#29374C', backgroundColor: '#060B12' },
  overviewRecoveryChartHeader: { minHeight: 58, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, paddingHorizontal: 13 },
  overviewRecoveryChartTitleRow: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 9 },
  overviewRecoveryChartIcon: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center', borderRadius: 11, backgroundColor: 'rgba(120,150,255,0.13)' },
  overviewRecoveryChartTitle: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.bodyBold, fontSize: 18 },
  overviewRecoveryChartRange: { color: '#C1CCE5', fontFamily: SLFontFamilies.body, fontSize: 16 },
  overviewReflectionCard: { position: 'relative', overflow: 'hidden', minHeight: 122, flexDirection: 'row', alignItems: 'center', gap: 16, paddingHorizontal: 20, paddingVertical: 16, borderWidth: 1, borderColor: '#5F3076', backgroundColor: '#0B0812' },
  overviewReflectionEdit: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: '#372441', backgroundColor: '#090711' },
  overviewReflectionEditText: { color: SLColors.accentMuted, fontFamily: SLFontFamilies.bodyBold, fontSize: 16 },
  overviewReflectionIcon: { width: 58, height: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: 'rgba(164,49,199,0.2)' },
  overviewReflectionCopy: { flex: 1, minWidth: 0 },
  overviewReflectionKicker: { color: '#D66EFF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16, lineHeight: 21, letterSpacing: 1.2 },
  overviewReflectionValue: { marginTop: 3, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 26, lineHeight: 30 },
  overviewReflectionDetail: { marginTop: 3, color: '#C5D0E8', fontFamily: SLFontFamilies.body, fontSize: 17, lineHeight: 23 },
  overviewNextActions: { gap: 12 },
  canonicalHero: { position: 'relative', minHeight: 354, overflow: 'hidden', borderBottomWidth: 1, borderBottomColor: '#3D2851', backgroundColor: '#090711' },
  canonicalHeroAnatomy: { position: 'absolute', top: -8, right: 0, width: 190, height: 220, alignItems: 'center', justifyContent: 'center', opacity: 0.54 },
  canonicalHeroArchiveArt: { position: 'absolute', top: -18, right: -28, width: 238, height: 238, opacity: 0.42 },
  canonicalHeroShade: { position: 'absolute', top: 0, left: 0, bottom: 124, width: '86%' },
  canonicalHeroCopy: { zIndex: 2, width: '78%', minHeight: 228, padding: 16 },
  completedBadge: { alignSelf: 'flex-start', minHeight: 26, flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 8, borderRadius: 7, borderWidth: 1, borderColor: 'rgba(76,175,116,0.32)', backgroundColor: 'rgba(24,92,58,0.12)' },
  completedBadgeText: { color: SLColors.success, fontFamily: SLFontFamilies.bodyBold, fontSize: 10, letterSpacing: 0.8 },
  canonicalHeroTitle: { marginTop: 11, color: '#F7F2FB', fontFamily: SLFontFamilies.display, fontSize: 30, lineHeight: 34 },
  sessionTimes: { marginTop: 10, color: '#C1B6CC', fontFamily: SLFontFamilies.body, fontSize: 13, lineHeight: 18 },
  canonicalHeroMetrics: { position: 'absolute', left: 0, right: 0, bottom: 0, minHeight: 124, flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 2, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(174,104,255,0.23)', backgroundColor: 'rgba(5,4,9,0.96)' },
  compactHighlightRail: { flexDirection: 'row', gap: 1, backgroundColor: '#241B2F' },
  compactHighlight: { flex: 1, minWidth: 0, minHeight: 116, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: 8, borderBottomWidth: 1, borderBottomColor: '#3D2A51', backgroundColor: '#0A0811' },
  compactHighlightArt: { width: 29, height: 29, marginBottom: 4, opacity: 0.82 }, compactHighlightCopy: { minWidth: 0, alignItems: 'center' }, compactHighlightLabel: { fontFamily: SLFontFamilies.bodyBold, fontSize: 10, letterSpacing: 0.45, textAlign: 'center' }, compactHighlightValue: { marginTop: 4, color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 20, textAlign: 'center' }, compactHighlightDetail: { marginTop: 4, color: '#B7ACBF', fontFamily: SLFontFamilies.body, fontSize: 11, lineHeight: 15, textAlign: 'center' },
  ...
  canonicalSection: { gap: 8 }, canonicalSectionHeading: { minHeight: 38, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: 10, paddingHorizontal: 8 }, canonicalSectionTitle: { color: '#C378FF', fontFamily: SLFontFamilies.bodyBold, fontSize: 16, letterSpacing: 0.8 }, canonicalSectionMeta: { flex: 1, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 20, textAlign: 'right' },
  ...
  bodyweightContextCard: { minHeight: 86, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: 14, borderRadius: 14, borderWidth: 1, borderColor: '#26475A', backgroundColor: '#071018' }, bodyweightContextValue: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 22 }, bodyweightContextDetail: { maxWidth: 280, marginTop: 4, color: SLColors.textMuted, fontFamily: SLFontFamilies.body, fontSize: 16, lineHeight: 21 }, overviewFeedbackAuthor: { fontSize: 18 }, overviewFeedbackMeta: { fontSize: 16, lineHeight: 21 }, overviewFeedbackQuote: { fontSize: 17, lineHeight: 24 },
