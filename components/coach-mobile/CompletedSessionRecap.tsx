...
import { SessionV3Closeout } from '@/components/workout-logger/session-v3-closeout';
...
type Props = {
  ...
  entryPresentation?: 'logger' | 'review';
...
export function CompletedSessionRecap({ recap, entryPresentation = 'review', onEditReflection, impactSummary, preferredUnits, refreshing, onRefresh, onClose, onDone, initialTab = 'overview', initialToolsOpen = false, initialScrollOffsetY = 0, initialExpandedItemId, viewerMode = 'athlete', sessionTimeZone, parentProvidesTopSafeArea = false, coachReview, coachReviewUnavailableReason, onViewLedger, onViewCalendar, onLogNextSession, onOpenProgramming, onOpenMovementHistory, onResumeSession, onCorrectEquipment, onEditSetEvidence, onEditSessionNotes, onViewSessionHistory }: Props) {
  ...
  if (entryPresentation === 'logger' && showCloseout) return <SessionV3Closeout
    recap={recap} movements={performedMovements.map(movement => {
      const best = movement.best_set;
      const result = best ? setResultLabel({ actual_weight_kg: best.weight_kg, actual_reps: best.reps }, movement, unit) : null;
      return { movement, result: [result, `${movement.sets.length} sets saved`].filter(Boolean).join(' · ') };
    })}
    date={dateLabel(recap.session.date)} duration={recap.session.duration_seconds == null ? '—' : durationLabel(recap.session.duration_seconds)}
    hasReflection={hasReflection} unit={unit} onUnit={() => setUnit(unit === 'kg' ? 'lb' : 'kg')}
    record={firstPrPresentation && firstPrMovement ? { label: firstPrMovement.label, classification: firstPrPresentation.classification, result: firstPrPresentation.currentLabel, detail: `${firstPrPresentation.deltaLabel} · ${firstPrPresentation.priorLabel}` } : null}
    onRecap={() => { setTab('overview'); setShowCloseout(false); }}
    onRecord={() => { setTab('personal_bests'); setShowCloseout(false); }}
    onHistory={onOpenMovementHistory} onReflection={onEditReflection}
    onDone={onDone || onClose} refreshing={refreshing} onRefresh={onRefresh}
    preview={viewerMode === 'coach'} topSafeArea={!parentProvidesTopSafeArea} />;
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row', alignItems: 'center', minHeight: 78, paddingHorizontal: 12, paddingBottom: 8, backgroundColor: '#020306' }, topButton: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderRadius: 13, borderWidth: 1, borderColor: SLColors.borderStandard, backgroundColor: '#080A10' }, topBarCopy: { flex: 1, minWidth: 0, alignItems: 'center', paddingHorizontal: 8 }, topTitle: { color: SLColors.textPrimary, fontFamily: SLFontFamilies.display, fontSize: 20 }, topDot: { color: SLColors.accentMuted }, topSubtitle: { marginTop: 2, color: SLColors.textSecondary, fontFamily: SLFontFamilies.body, fontSize: 16 }, completeMark: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderRadius: 13, borderWidth: 1, borderColor: SLColors.borderStandard, backgroundColor: '#0B0D14' },
  ...
  overviewSessionHero: { position: 'relative', minHeight: 296, overflow: 'hidden', flexDirection: 'row', borderWidth: 1, borderColor: '#007E51', backgroundColor: '#03100E' },
  ...
  overviewSessionHeroIcon: { width: 56, height: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 17, borderWidth: 1, borderColor: 'rgba(0,233,144,0.18)', backgroundColor: 'rgba(0,145,85,0.25)' },
  ...
  canonicalHeroAnatomy: { position: 'absolute', top: 8, right: 6, width: 150, height: 160, alignItems: 'center', justifyContent: 'center' },
