...
export function StrengthHero({ data, unit, compact = false, onPress, onLiftPress }: { data: CoachingPerformance; unit: LedgerUnit; compact?: boolean; onPress?: () => void; onLiftPress?: (key: string) => void }) {
  ...
  return <View style={[v.hero, compact && v.compactHero]}>
    <Image accessible={false} source={LEDGER_INDEX_ASSETS.hero} resizeMode="contain" style={[v.heroArt, compact && v.compactArt]} />
    <LinearGradient colors={['rgba(15,9,24,0.08)', '#08060E']} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={StyleSheet.absoluteFill} />
    <View style={v.heroContent}>
      <Text style={v.eyebrow}>ESTIMATED TOTAL</Text>
      <View style={v.totalLine}><Text style={[v.total, compact && v.compactTotal]}>{displayCalculatedWeight(arc?.estimated_total_kg, unit)}</Text><Text style={v.totalUnit}>{unit}</Text></View>
      <View style={v.deltaLine}><View style={[v.signalDot, { backgroundColor: delta == null ? INK.quiet : delta < 0 ? INK.magenta : INK.green }]} /><Text style={[v.delta, { color: delta == null ? INK.muted : delta < 0 ? INK.magenta : INK.green }]}>{deltaLabel(delta, unit)}</Text>{delta != null ? <Text style={v.meta}>in this period</Text> : null}</View>
      ...
      {!compact && lifts.some((lift) => lift.points?.length) ? <View style={v.heroChart}>
        <AnalyticalTimeSeriesChart testID="coach-performance-strength-chart" height={208} readableText selectedInitially="none" showLegend={false}
      ...
      <View style={v.liftStrip}>{lifts.map((lift) => <Pressable key={lift.key} accessibilityRole="button" accessibilityLabel={`Open ${lift.label} strength evidence`} onPress={() => onLiftPress?.(lift.key!)} style={({ pressed }) => [v.lift, pressed && v.pressed]}>
        ...
        <Text style={v.liftValue}>{displayCalculatedWeight(lift.current_e1rm_kg, unit)}<Text style={v.smallUnit}> {unit}</Text></Text>
...
export const v = StyleSheet.create({
  ...
  heroArt: { position: 'absolute', right: -30, top: -32, width: 245, height: 245, opacity: 0.64 },
  compactArt: { width: 190, height: 190, right: -15, top: -26, opacity: 0.42 },
  heroContent: { padding: 19 },
  totalLine: { flexDirection: 'row', alignItems: 'baseline', gap: 8, marginTop: 3 },
  total: { color: INK.text, fontFamily: SLFontFamilies.numeric, fontSize: 61, letterSpacing: -2 },
  compactTotal: { fontSize: 48 },
  ...
  lift: { flex: 1, gap: 4, minHeight: 52 },
  liftLabelRow: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  liftLabel: { color: '#C5BED1', fontSize: 13 },
  liftValue: { color: INK.text, fontFamily: SLFontFamilies.numeric, fontSize: 24 },
