...
export function deltaLabel(value: number | null | undefined, unit: LedgerUnit) {
  return value == null ? 'No comparison yet' : `${value > 0 ? '+' : value < 0 ? '−' : ''}${loadLabel(Math.abs(value), unit)}`;
}
export function Chapter({ title, detail, action, onPress, children }: { title: string; detail?: string; action?: string; onPress?: () => void; children: React.ReactNode }) {
...
export function EvidenceLink({ title, detail, quiet = false, onPress }: { title: string; detail?: string; quiet?: boolean; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [v.evidenceLink, quiet && v.quietLink, pressed && v.pressed]}><View style={v.flex}><Text style={v.rowTitle}>{title}</Text>{detail ? <Text style={v.body}>{detail}</Text> : null}</View><Ionicons color={INK.violet} name="arrow-forward" size={21} /></Pressable>;
}
...
export function StrengthHero({ data, unit, compact = false, dense = false, onPress, onLiftPress }: { data: CoachingPerformance; unit: LedgerUnit; compact?: boolean; dense?: boolean; onPress?: () => void; onLiftPress?: (key: string) => void }) {
  ...
  return <View style={[v.hero, compact && v.compactHero]}>
    ...
    <View style={[v.heroContent, compact && v.compactContent, dense && v.denseContent]}>
      <Text style={v.eyebrow}>{title}</Text>
      <View style={v.totalLine}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[v.total, compact && v.compactTotal, headline == null && v.emptyTotal]}>{headline == null ? "No recorded strength" : displayCalculatedWeight(headline, unit)}</Text>{headline != null ? <Text style={v.totalUnit}>{unit}</Text> : null}</View>
      ...
      {!compact && !dense ? <Text style={v.heroContext}>{arc?.estimated_total_kg == null ? 'A Total appears when all three competition lifts have evidence.' : 'Latest weekly bests · competition S / B / D'}</Text> : null}
      {!compact && lifts.some((lift) => lift.points?.length) ? <View style={v.heroChart}>
        <AnalyticalTimeSeriesChart testID="coach-performance-strength-chart" height={dense ? 168 : 178} readableText selectedInitially="none" showLegend={false}
      ...
      <View style={[v.liftStrip, compact && v.compactStrip, dense && v.denseStrip]}>{lifts.map((lift) => <Pressable key={lift.key} accessibilityRole="button" accessibilityLabel={`Open ${lift.label} strength evidence`} onPress={() => onLiftPress?.(lift.key!)} style={({ pressed }) => [v.lift, pressed && v.pressed]}>
...
export const v = StyleSheet.create({
  ...
  chapter: { gap: 9, paddingTop: 14, marginTop: 7, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: INK.line },
  ...
  quietLink: { borderTopWidth: 0, paddingVertical: 10, minHeight: 44 },
  ...
  denseContent: { padding: 14 },
  denseStrip: { marginTop: 6, paddingTop: 10 },
