...
import { coachMeetTimingLabel, formatCoachMeetDate, normalizeCoachMeetContext } from '@/lib/coach-meet-day';
...
export function CoachAthleteHubV2({ previewSummary }: { previewSummary?: CoachAthleteSummaryResponse }) {
  ...
  const meet = normalizeCoachMeetContext(summary?.meet_context);
    ...
    <FloatingControlCoordinator context="screen">
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} tintColor={COACH_V2.violet} onRefresh={() => load({ silent: true })} />} showsVerticalScrollIndicator={false}>
        ...
        {summary ? <>
          ...
          {meet ? <Pressable accessibilityLabel={`Open ${summary.athlete.name} Meet Day in Calendar`} accessibilityRole="button" onPress={() => router.push({ pathname: '/(tabs)/coach-calendar', params: { athleteId: String(summary.athlete.id) } } as any)} style={styles.meetDay}><View style={styles.meetDayIcon}><Ionicons color={COACH_V2.gold} name="trophy" size={24} /></View><View style={styles.flex}><Text style={styles.meetDayLabel}>MEET DAY · {coachMeetTimingLabel(meet)}</Text><Text numberOfLines={1} style={styles.meetDayTitle}>{meet.meet_name || 'Meet Day'}</Text><Text style={styles.meta}>{formatCoachMeetDate(meet.meet_date)}</Text></View><Ionicons color={COACH_V2.gold} name="chevron-forward" size={18} /></Pressable> : null}
...
const styles = StyleSheet.create({
  ...
  meetDay: { minHeight: 78, flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 12, borderWidth: 1, borderColor: '#67531F', backgroundColor: '#151005', padding: 11 }, meetDayIcon: { width: 44, height: 44, borderRadius: 22, borderWidth: 1, borderColor: '#725D24', backgroundColor: '#241B06', alignItems: 'center', justifyContent: 'center' }, meetDayLabel: { color: COACH_V2.gold, fontSize: 8.5, fontWeight: '900', letterSpacing: 0.45 }, meetDayTitle: { color: COACH_V2.text, fontSize: 14, lineHeight: 18, fontWeight: '800' },
