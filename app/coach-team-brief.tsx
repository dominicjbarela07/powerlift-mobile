...
import { AppState, Image, RefreshControl, StyleSheet, View } from 'react-native';
...
import { presentTeamBriefHighlight } from '@/lib/team-brief-highlights';
...
export default function CoachTeamBriefScreen() {
  ...
  return (
    <SLScreen contentStyle={styles.screen} edges="top" padded={false}>
      <SLContextualHeader
        action={{ accessibilityLabel: 'Open Team Brief methodology', icon: 'reader-outline', onPress: () => router.push('/coach-team-methodology' as any) }}
        backAccessibilityLabel="Close Team Brief"
        breadcrumb="The Coach's Ledger"
        onBack={close}
        title="Team Brief"
      />
      <SLCompactTabRail
        items={PERIODS.map((key) => ({ key, label: key }))}
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl onRefresh={() => { setRefreshing(true); void load({ silent: true }); }} refreshing={refreshing} tintColor={COACH_V2.violetBright} />} showsVerticalScrollIndicator={false}>
        ...
        {brief ? <>
          ...
          <SectionHeader index="7" subtitle="Milestones backed by persisted evidence" title="RECENT HIGHLIGHTS" /><Highlights onOpen={(row) => row.destination && router.push(row.destination as any)} rows={brief.highlights} />
...
function Highlights({ onOpen, rows }: { onOpen: (row: CoachTeamBriefResponse['highlights'][number]) => void; rows: CoachTeamBriefResponse['highlights'] }) {
  ...
  return <Card tone="on_track">{rows.map((row, index) => { const copy = presentTeamBriefHighlight(row); const enabled = Boolean(row.destination); return <Pressable key={row.key} accessibilityLabel={row.type === 'pr' ? `Open ${row.title} ${copy.badge || 'PR'} evidence` : row.title} accessibilityRole={enabled ? 'button' : undefined} disabled={!enabled} onPress={() => onOpen(row)} style={({ pressed }) => [styles.highlightRow, index > 0 && styles.highlightDivider, pressed && styles.pressed]}><Image source={row.asset === 'session_streak_medallion' ? require('@/assets/images/session-recap/session-streak-medallion-v1.png') : require('@/assets/images/ledger-index-v2/ledger-career-pr-medallion-v1.png')} style={styles.highlightImage} /><View style={styles.flexOne}><View style={styles.highlightHeading}><Text numberOfLines={1} style={styles.highlightTitle}>{copy.title}</Text>{copy.badge ? <Text style={styles.highlightBadge}>{copy.badge}</Text> : null}</View><Text numberOfLines={1} style={styles.highlightResult}>{copy.primary}</Text>{copy.comparison ? <Text numberOfLines={2} style={styles.highlightCopy}>{copy.comparison}</Text> : null}{copy.date ? <Text style={styles.highlightDate}>{copy.date}{row.record_count && row.record_count > 1 ? ` · ${row.record_count} records` : ''}</Text> : null}</View>{enabled ? <Ionicons color={COACH_V2.subtle} name="chevron-forward" size={16} /> : null}</Pressable>; })}</Card>;
}

const styles = StyleSheet.create({
  screen: { backgroundColor: '#000' }, content: { paddingTop: 12, gap: 10 },
  ...
  impactGrid: { flexDirection: 'row', flexWrap: 'wrap' }, impactCell: { width: '25%', minHeight: 74, alignItems: 'center', justifyContent: 'center', padding: 3, borderRightWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: '#252A34' }, impactValue: { color: COACH_V2.text, fontSize: 19, fontWeight: '800' }, impactLabel: { color: COACH_V2.muted, fontSize: 7, lineHeight: 9, textAlign: 'center', marginTop: 4 }, impactNote: { color: COACH_V2.subtle, fontSize: 8, lineHeight: 12, marginTop: 10 }, highlightRow: { minHeight: 92, flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8 }, highlightDivider: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#262A33' }, highlightImage: { width: 42, height: 42, resizeMode: 'contain' }, highlightHeading: { flexDirection: 'row', alignItems: 'center', gap: 6 }, highlightTitle: { flex: 1, color: COACH_V2.text, fontSize: 11, fontWeight: '800' }, highlightBadge: { color: COACH_V2.gold, borderWidth: 1, borderColor: `${COACH_V2.gold}66`, borderRadius: 6, paddingHorizontal: 5, paddingVertical: 2, fontSize: 7, fontWeight: '900' }, highlightResult: { color: COACH_V2.gold, fontSize: 10, fontWeight: '800', marginTop: 4 }, highlightCopy: { color: COACH_V2.muted, fontSize: 9, lineHeight: 12, marginTop: 2 }, highlightDate: { color: COACH_V2.subtle, fontSize: 8, marginTop: 3 }, qualityHeader: { flexDirection: 'row', alignItems: 'center', gap: 11, marginBottom: 10 }, qualityTitle: { color: COACH_V2.text, fontSize: 13, fontWeight: '800' }, qualityCopy: { color: COACH_V2.muted, fontSize: 9, lineHeight: 13, marginTop: 2 }, qualityNote: { color: COACH_V2.subtle, fontSize: 9, lineHeight: 14, marginTop: 3 }, flexOne: { flex: 1, minWidth: 0 }, pressed: { opacity: 0.72, transform: [{ scale: 0.985 }] }, bottomSpace: { height: 48 },
});
